# 6v6 보안 실습 인프라 — 완전 재구축 프롬프트 (REBUILD PROMPT)

> 작성일: 2026-06-03 · 대상 저장소: https://github.com/mrgrit/6v6 · 기준 커밋: `46e1685`
>
> **이 문서의 목적**: 이 한 문서만으로 6v6 프로젝트 전체를 빈 디렉토리에서 재구축할 수 있어야 한다.
...
당신은 시니어 보안 인프라 엔지니어다. 아래 명세를 받아 `6v6/` 저장소를 처음부터 만든다.
- 언어/주석: 한국어 중심 (코드 식별자는 영어). 기존 톤(교육용, 친절한 안내 메시지) 유지.
- 모든 컨테이너는 단일 Linux VM(VMware Bridge/NAT) 위 Docker Compose 로 구동.
- "학생 신규 배포" 자동화가 핵심 — `bash 6v6.sh install && bash 6v6.sh up` 만으로 동작해야 함.
...
3. **ips MASQUERADE**: ips 가 dmz 로 나가는 트래픽(출발: ext/pipe/user)을 dmz 인터페이스로 SNAT (`nftables nat6v6` 테이블 POSTROUTING) → backend 가 ips 를 GW 로 인식, 역경로 불필요.
4. **호스트 iptables DOCKER-USER**: `6v6.sh` 의 `cmd_setup_forward` 가 `up` 후 브리지 인터페이스 이름을 조회해 `DOCKER-USER` 체인에 `ext↔pipe`, `pipe↔dmz`, `dmz↔int` 양방향 ACCEPT 규칙을 삽입하고 끝에 `RETURN`.
...
├── keys/                        # .gitkeep 만 추적 (id_rsa[.pub] 는 6v6.sh 가 자동 생성, gitignore)
...
- `ensure_ssh_keys`: `keys/` 생성, `keys/id_rsa` 없으면 `ssh-keygen -t ed25519 -f keys/id_rsa -N "" -C 6v6-bastion@auto`. 권한 600/644. **학생 환경마다 다른 키** (gitignore). 6개 컨테이너(bastion/attacker/fw/ips/web/siem)가 `./keys:/keys:ro` 마운트 → bastion 의 ccc 가 개인키 보유, 나머지는 pub 을 authorized_keys 로 받아 password 없는 ProxyJump.
...
`name: 6v6`. 섹션 2.2 IP 표, 섹션 2.1 네트워크, 아래 볼륨/환경을 그대로 구현.

**서비스별 핵심**:
- **bastion**: build `bastion/Dockerfile`, ext .201, port `${PORT_BASTION_SSH:-2204}:22`, `cap_add:[NET_ADMIN]`. env: `SSH_USER/SSH_PASS/API_KEY`, `LLM_BASE_URL=${LLM_BASE_URL:-http://192.168.0.109:11434}`, `LLM_MANAGER_MODEL=${LLM_MANAGER_MODEL}`, `LLM_SUBAGENT_MODEL=${LLM_SUBAGENT_MODEL}`(fallback 없음 — 미설정시 startup 실패로 즉시 인지), `DEFAULT_GW=10.20.30.1`. `extra_hosts`: 모든 `*.6v6.lab → 10.20.30.1`(fw HAProxy). volumes: `bastion-data:/var/lib/bastion`, `bastion-ssh-host:/var/lib/bastion/ssh-host-keys`, `/var/run/docker.sock:ro`, `./keys:/keys:ro`.
...
- **fw**: build `fw/Dockerfile`, ext .1(+alias secu,6v6-secu) / pipe .1(+alias). ports `80:80`, `443:443`, `9100:9100`. `cap_add:[NET_ADMIN,NET_RAW]`, `sysctls: net.ipv4.ip_forward=1`. env: `WAZUH_MANAGER=10.20.32.100`, `IPS_PIPE_IP=10.20.31.2`. `./keys:ro`.
...
- entrypoint: ext 복귀 route via `FW_PIPE_IP`(10.20.31.1), **nat6v6 테이블 POSTROUTING masquerade**(출발 10.20.30.0/24, 10.20.31.0/24, 10.20.33.0/24 → dmz 인터페이스), Suricata af-packet 듀얼(pipe+dmz) 기동, eve.json(json, stats 비활성-필드 256 초과 방지), wazuh-agent(eve.json json + syslog), sshd.
...
- ModSecurity: `SecRuleEngine On`(단 juice.6v6.lab vhost 는 `DetectionOnly` — W02/W03 학습), `SecAuditEngine RelevantOnly`, `SecAuditLog /var/log/apache2/modsec_audit.log`, JSON 포맷, parts ABCFHZ.
- self-signed cert `*.6v6.lab` → `/etc/apache2/ssl/`. landing DocumentRoot `/var/www/landing/`.
- **vhosts/** (파일명 = 우선순위): `000-landing`(6v6.lab→landing), `010-juice`(juice.6v6.lab→juiceshop:3000, /socket.io WS, DetectionOnly), `020-dvwa`(→dvwa:80), `030-neobank`(→neobank:3001), `040-govportal`(→govportal:3002), `050-mediforum`(→mediforum:3003), `060-admin`(admin.6v6.lab→adminconsole:3004), `070-ai`(ai.6v6.lab→aicompanion:3005), `080-portal`(portal.6v6.lab→portal:8000, ModSec off), `090-siem`(siem.6v6.lab→wazuh-dashboard:5601 HTTPS SSLProxyEngine verify none), `100-bastion`(bastion.6v6.lab→bastion:9100). 모든 vhost ProxyPreserveHost On + 개별 access/error 로그.
...
### 10.2 SSH ProxyJump
`~/.ssh/config`: `6v6-bastion`(port 2204 ccc), `6v6-attacker`(port 2202 ccc), `6v6-fw 6v6-ips 6v6-web 6v6-siem 6v6-portal 6v6-win`(ProxyJump 6v6-bastion). bastion 내부 alias: `ssh fw`(10.20.30.1), `ssh ips`(10.20.31.2), `ssh web`(10.20.32.80), `ssh siem`(10.20.32.100), `ssh attacker`(10.20.30.202), `ssh win`(10.20.33.60).
...
`dockurr/windows`, 6v6-win, user .60. env VERSION tiny11/RAM_SIZE 4G/CPU_CORES 2/DISK_SIZE 48G/USERNAME ccc/PASSWORD ccc. devices `/dev/kvm`,`/dev/net/tun`. 포트 8006(VNC)/3389(RDP). 마운트 win-storage:/storage, win-oem:/oem, win-shared:/data. network 6v6-user(external).
**install.bat (OEM 무인설치)**: ① 정적 라우팅(ext/pipe/dmz/int → 10.20.33.1 ips) ② Sysmon64 + SwiftOnSecurity config ③ Wazuh agent 4.10.0 MSI(WAZUH_MANAGER=10.20.32.100, name 6v6-win) + agent-auth + Sysmon eventchannel localfile 주입 ④ OpenSSH Win64 v9.8.1.0p1(기본셸 PowerShell, 방화벽 22 허용, sshd auto) ⑤ hosts `*.6v6.lab→10.20.32.80` ⑥ ICMP 허용 + Wazuh 재기동 ⑦ 완료마커 `\\host.lan\Data\OEM_DONE.txt`. 첫 부팅 30-60분. `cmd_win_route_fix` 가 default route 를 ips 로 교체.
...
```bash
git clone https://github.com/mrgrit/6v6 && cd 6v6
bash 6v6.sh install        # docker+compose+helpers (Debian/Ubuntu), docker group 추가
newgrp docker              # 또는 새 터미널
cp .env.example .env       # (6v6.sh 가 자동으로도 함)
bash 6v6.sh up                  # base 15 + overlay + secuops-easy GUI (첫 빌드 20-30분, ~15GB)
bash 6v6.sh up --with-windows   # + Windows tiny11 (KVM, 첫 부팅 +30-60분)
bash 6v6.sh smoke          # 헬스 + Wazuh agent 등록 검증
bash 6v6.sh status         # VM_IP/포트/SSH 안내
```
